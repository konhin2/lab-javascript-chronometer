...
function printMilliseconds() {
  ...
  const contar = () => {
    currentMili++;
    if (currentMili < 10) {
      milUniElement.innerHTML = `${currentMili}`;
      milDecElement.innerHTML = '0';
    } else {
      let someString = `${currentMili}`;
      milUniElement.innerHTML = someString[1];
      milDecElement.innerHTML = someString[0];
    }
    if (currentMili > 99) {
      currentMili = 0;
    }
  };
  intervId = setInterval(contar, 10);
}
...
function printSplit() {
  ...
  if (currentMili < 10) {
    li.innerText = `${chronometer.split()}:0${currentMili}`;
  } else {
    li.innerText = `${chronometer.split()}:${currentMili}`;
  }
